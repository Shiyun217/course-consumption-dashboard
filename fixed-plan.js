(function () {
  ...
  function renderLatest() {
    ...
    return `${navMarkup()}<section class="fp-latest-hero"><div><div class="kicker">LATEST FIXED PLAN PULSE</div><h1>M0-1固定计划绑定-最新</h1><p>聚焦当前整体进度、三端口目标差距与14个固定小组的补齐优先级</p></div><div class="fp-latest-date"><span>数据更新</span><b>${latestData.source_as_of}</b><small>${latestData.period}</small></div></section>
      ...
      <div class="fp-latest-source-note"><b>对比说明：</b>整体指标已按 0828 明细有效绑定口径更新；CC / SS / LP 仍保留原数据，昨日值来自 0827 同口径结果。整体及上月同期因当前数据源未提供历史快照，暂不展示差值。</div>
  ...
  function setFixedToolbar() {
    ...
    if (importButton) importButton.onclick = () => alert(fixedTab === 'latest' ? '最新数据源：BI港澳SS日常数据看板（最近快照截至0825）、学员付费明细_0828.xlsx、固定计划绑定人明细_828.xlsx。' : '数据源：业务确认整体固定率表、学员付费明细_0807.xlsx、固定计划绑定人历史数据.xlsx。');
  }
