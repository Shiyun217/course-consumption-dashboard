(function () {
  ...
  function renderM01() {
    ...
    const latestMonthNumber = latest ? Number(latest.month.slice(-2)) : 0;
    const overall2026ToLatest = overall.filter(row => row.month >= '2026-01' && row.month <= latest.month);
    const avg2025 = mean(overall2025.map(row => row.rate));
    const avg2026ToLatest = mean(overall2026ToLatest.map(row => row.rate));
    ...
    const previousOverall = overall.length > 1 ? overall[overall.length - 2] : null;
    const latestOperatorMonth = latestOperator ? latestOperator.month : '暂无';
    ...
    return `${navMarkup()}<section class="fp-hero"><div><div class="kicker">FIXED PLAN PERFORMANCE · HISTORY</div><h1>M0-1固定计划绑定（历史）</h1><p class="subtitle">整体趋势使用业务确认口径；端口分析按滚动双月新生 cohort 与全历史首次绑定口径计算</p></div><div class="fp-scope"><div class="fp-scope-row"><span>整体趋势</span><b>2025-01 至 ${latest.month}</b></div><div class="fp-scope-row"><span>端口分析</span><b>2025-02 至 ${latestOperatorMonth}</b></div><div class="fp-scope-row"><span>端口分母</span><b>当月滚动双月新生数</b></div></div></section>
      <section class="fp-kpis"><div class="fp-kpi"><div class="fp-kpi-label"><span>2025月均固定率</span><span>12个月</span></div><div class="fp-kpi-value">${pct(avg2025)}</div><div class="fp-kpi-meta">整体固定计划绑定率的月度简单平均</div></div><div class="fp-kpi"><div class="fp-kpi-label"><span>2026年1-${latestMonthNumber}月均值</span><span>截至最新月份</span></div><div class="fp-kpi-value">${pct(avg2026ToLatest)}</div><div class="fp-kpi-meta">较2025月均 ${pp(avg2026ToLatest-avg2025)}</div></div><div class="fp-kpi"><div class="fp-kpi-label"><span>最高月份</span><span>${peak.month}</span></div><div class="fp-kpi-value">${pct(peak.rate)}</div><div class="fp-kpi-meta">历史高点仍低于65%目标 ${pp(peak.rate-.65)}</div></div><div class="fp-kpi"><div class="fp-kpi-label"><span>最新月份</span><span>${latest.month}</span></div><div class="fp-kpi-value">${pct(latest.rate,2)}</div><div class="fp-kpi-meta">${previousOverall ? `较${previousOverall.month} ${pp(latest.rate-previousOverall.rate,2)}` : '暂无上月对比'}，需关注连续性</div></div></section>
      <section class="fp-trend-layout"><div class="panel fp-trend-panel"><div class="panel-head"><div><div class="panel-title">整体固定计划绑定率趋势</div><div class="panel-sub">2025-01 至 ${latest.month}，每个点均展示月度固定率；虚线为65%目标</div></div></div><div class="panel-body">${lineChart(overall,{min:.2,max:.7,showEveryValue:true,valueDigits:2,target:.65,chartWidth:920,chartHeight:460,preserveAspect:true,className:'fp-overall-chart',ariaLabel:`2025年1月至${latest.month}整体固定计划绑定率`})}</div></div>
      <aside class="fp-trend-insights" aria-label="整体趋势分析"><article class="fp-insight"><span>阶段改善</span><b>2026年截至${latest.month}高于2025</b><p>2026年1-${latestMonthNumber}月均${pct(avg2026ToLatest)}，较2025年月均${pct(avg2025)}提升${pp(avg2026ToLatest-avg2025)}。8月回升至${pct(latest.rate,1)}，但仍需观察是否形成稳定平台。</p></article><article class="fp-insight"><span>最新变化</span><b>${latest.month}较前月${pp(latest.rate-(previousOverall?.rate || latest.rate),1)}</b><p>最新月份固定率为${pct(latest.rate,1)}，${previousOverall ? `较${previousOverall.month}的${pct(previousOverall.rate,1)}变化${pp(latest.rate-previousOverall.rate,1)}` : '暂无上月可比数据'}。建议把回升动作固化为月内节点机制，避免依赖阶段性冲刺。</p></article><article class="fp-insight"><span>历史低点</span><b>${trough.month}仅${pct(trough.rate)}</b><p>2025年初从25.0%快速修复，但2025年7-8月与2026年1-2月仍出现低谷，说明淡旺季、责任切换和补漏节奏需要纳入固定SOP。</p></article></aside></section>
      <section class="panel fp-section" id="operatorSection"><div class="panel-head"><div><div class="panel-title">M0-1首次固定操作人固定率趋势</div><div class="panel-sub">端口固定率 = 该端口首次固定学员数 / 当月新生数；各端口合计等于整体固定率（端口数据截至${latestOperatorMonth}）</div></div>${operatorLegend()}</div><div class="panel-body">${lineChart(operatorRows,{series:operatorSeries,min:0,max:.35,ariaLabel:`2025年2月至${latestOperatorMonth}各端口首次固定率趋势`})}</div>${operatorTable()}</section>
      <section class="fp-analysis-grid"><article><span>CC：${latestOperatorMonth}仍是最大端口</span><b>${pct(latestOperator.rates.CC)} / 目标 ${pct(target.CC,0)}</b><p>${latestOperatorMonth}CC固定率${pct(latestOperator.rates.CC,1)}，仍是三大端口中贡献最高的一端。要达到23%管理目标，需要提升${pp(target.CC-latestOperator.rates.CC)}，承担最大的绝对增量。</p></article><article><span>SS：${latestOperatorMonth}端口表现</span><b>${pct(latestOperator.rates.SS)} / 目标 ${pct(target.SS,0)}</b><p>${latestOperatorMonth}SS固定率${pct(latestOperator.rates.SS,1)}，建议把20%作为稳定底线，当前仍需提升${pp(target.SS-latestOperator.rates.SS)}。</p></article><article><span>LP：优先恢复有效机制</span><b>${pct(latestOperator.rates.LP)} / 目标 ${pct(target.LP,0)}</b><p>LP当前固定率${pct(latestOperator.rates.LP,1)}，17%目标低于其历史高点，但仍需提升${pp(target.LP-latestOperator.rates.LP)}，重点是恢复有效动作而非继续压高指标。</p></article><article><span>student / other：保留辅助贡献</span><b>${pct(latestOperator.rates.student + latestOperator.rates.other)}基线</b><p>${latestOperatorMonth}student与other合计${pct(latestOperator.rates.student + latestOperator.rates.other,1)}。目标拆解假设两者维持当前贡献，不把65%的主要增量压力转移给这两个辅助端口。</p></article></section>
      ${targetBlock()}`;
  }
  ...
  function m01AllTargetBlock(data) {
    ...
    const baselineLabel=monthLabel(data.target.baseline_period);
    ...
    const cards=['CC','SS','LP'].map(port=>`<article class="fp-target-card"><div class="fp-target-head"><span>${port}</span><b>${pct(recommended[port],0)}</b></div><div class="fp-target-track"><span style="width:${Math.min(baseline[port]/recommended[port],1)*100}%;background:${colors[port]}"></span></div><div class="fp-target-meta"><span>${baselineLabel} ${pct(baseline[port])}</span><strong>需提升 ${pp(recommended[port]-baseline[port])}</strong></div></article>`).join('');
    return `<section class="panel fp-section" id="fpAllTarget"><div class="panel-head"><div><div class="panel-title">去重65%目标 → 不去重目标拆解</div><div class="panel-sub">先按${baselineLabel}去重转化效率反推不去重目标，再按CC、SS、LP当前贡献结构分摊</div></div><span class="fp-goal-chip">去重目标 65%</span></div><div class="panel-body"><div class="fp-target-summary"><div><span>${baselineLabel}去重覆盖</span><b>${pct(baseline.total)}</b></div><div><span>${baselineLabel}不去重固定率</span><b>${pct(baseline.touch_sum)}</b></div><div><span>反推不去重目标</span><b>${pct(data.target.required_touch_sum)}</b></div><div><span>预计去重覆盖</span><b>${pct(data.target.estimated_total)}</b></div></div><div class="fp-target-grid">${cards}</div><div class="fp-target-rationale"><b>建议目标：CC 33% / SS 38% / LP 25% = 96%</b><p>${baselineLabel}去重覆盖${pct(baseline.total)}、不去重固定率${pct(baseline.touch_sum)}，对应去重转化效率${pct(baseline.union_efficiency)}。若该效率保持不变，要实现去重覆盖65%，不去重固定率需达到约${pct(data.target.required_touch_sum)}。按${baselineLabel}CC、SS、LP当前贡献结构精确分摊约为CC ${pct(exact.CC)}、SS ${pct(exact.SS)}、LP ${pct(exact.LP)}；取带缓冲的整数目标33% / 38% / 25%，三端合计${pct(targetTouchSum)}，预计对应去重覆盖${pct(data.target.estimated_total)}。对应CC需提升${pp(recommended.CC-baseline.CC)}、SS需提升${pp(recommended.SS-baseline.SS)}、LP需提升${pp(recommended.LP-baseline.LP)}；student与other作为额外贡献，不计入三端目标。</p></div></div></section>`;
  }

  function renderM01All() {
    if(!m01AllData) return renderEmpty('M0-1固定计划绑定（不去重）');
    const rows=m01AllData.rows,ports=m01AllData.ports,latest=rows[rows.length-1],previous=rows[rows.length-2],latestLabel=monthLabel(latest.month);
    ...
    return `${navMarkup()}<section class="fp-hero"><div><div class="kicker">M0-1 ALL PORT TOUCHES</div><h1>M0-1固定计划绑定（不去重）</h1><p class="subtitle">同一学员在同一端口多次操作只计1人；跨端口操作分别计入各端口，整体覆盖人数仍按学员去重</p></div><div class="fp-scope"><div class="fp-scope-row"><span>统计范围</span><b>${m01AllData.range.start} 至 ${m01AllData.range.end}</b></div><div class="fp-scope-row"><span>统计窗口</span><b>当月 + 上月</b></div><div class="fp-scope-row"><span>端口分母</span><b>滚动双月新生数</b></div></div></section>
      <div class="fp-note"><b>阅读提示：</b>本看板的不去重固定率 = CC + SS + LP + student + other端口率。同一学员可跨端口重复贡献，因此该指标会高于去重覆盖率；同一学员在同一端口无论操作多少次只计1人。</div>
      <section class="fp-kpis"><div class="fp-kpi"><div class="fp-kpi-label"><span>${latestLabel}不去重固定率</span><span>${num(latest.touch_sum)}次触达</span></div><div class="fp-kpi-value">${pct(latest.touch_sum_rate)}</div><div class="fp-kpi-meta">五端口触达率之和，已高于65%目标${pp(latest.touch_sum_rate-.65)}</div></div><div class="fp-kpi"><div class="fp-kpi-label"><span>${latestLabel}去重覆盖参考</span><span>${num(latest.bound_students)}人</span></div><div class="fp-kpi-value">${pct(latest.binding_rate)}</div><div class="fp-kpi-meta">同一学员跨端口只计1人</div></div><div class="fp-kpi"><div class="fp-kpi-label"><span>${latestLabel}跨端口重复</span><span>${num(latest.overlap_touches)}次</span></div><div class="fp-kpi-value">${pct(latest.overlap_rate)}</div><div class="fp-kpi-meta">不去重固定率减去去重覆盖率</div></div><div class="fp-kpi"><div class="fp-kpi-label"><span>历史最高不去重固定率</span><span>${rows.reduce((best,row)=>row.touch_sum_rate>best.touch_sum_rate?row:best,rows[0]).month}</span></div><div class="fp-kpi-value">${pct(Math.max(...rows.map(row=>row.touch_sum_rate)))}</div><div class="fp-kpi-meta">端口协同触达强度的历史高点</div></div></section>
      <section class="panel fp-section"><div class="panel-head"><div><div class="panel-title">不去重固定率与去重覆盖参考</div><div class="panel-sub">深色线为五端口率之和并用于衡量65%目标；灰线为去重学员覆盖率</div></div></div><div class="panel-body">${lineChart(rows,{series:coverageSeries,min:.3,max:.9,showEveryValue:true,valueDigits:1,target:.65,ariaLabel:`${m01AllData.range.start}至${m01AllData.range.end}不去重固定率与去重覆盖参考趋势`})}</div></section>
      <section class="panel fp-section" id="operatorAllSection"><div class="panel-head"><div><div class="panel-title">M0-1各端口触达率趋势</div><div class="panel-sub">端口触达率 = 该端口在M0-1出现过绑定动作的学员数 / 滚动双月新生数；学员×端口仅计1次</div></div>${operatorLegend()}</div><div class="panel-body">${lineChart(rows,{series,min:0,max:.4,ariaLabel:`${m01AllData.range.start}至${m01AllData.range.end}M0-1不去重端口触达率趋势`})}</div>${m01AllTable(rows,ports)}</section>
      <section class="fp-analysis-grid"><article><span>不去重目标</span><b>去重65%需约${pct(m01AllData.target.required_touch_sum)}</b><p>${latestLabel}不去重固定率${pct(latest.touch_sum_rate)}，按基准转化效率反推仍需提升${pp(m01AllData.target.required_touch_sum-latest.touch_sum_rate)}。</p></article><article><span>端口触达</span><b>SS ${pct(latest.rates.SS)}最高</b><p>${latestLabel}CC ${pct(latest.rates.CC)}、SS ${pct(latest.rates.SS)}、LP ${pct(latest.rates.LP)}；对应管理目标为33%、38%、25%。</p></article><article><span>重复触达</span><b>${pct(latest.overlap_rate)}来自跨端口重叠</b><p>${latestLabel}${num(latest.bound_students)}名固定学员产生${num(latest.touch_sum)}次端口触达，额外${num(latest.overlap_touches)}次来自同一学员被多个端口操作。</p></article><article><span>管理结论</span><b>三端目标合计96%</b><p>CC需提升${pp(.33-latest.rates.CC)}、SS需提升${pp(.38-latest.rates.SS)}、LP需提升${pp(.25-latest.rates.LP)}。同时需维持当前端口重叠效率，才能支撑去重覆盖65%。</p></article></section>
